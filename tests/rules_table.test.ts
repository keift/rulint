...
const eslint_plugin_zod_rule_descriptions = Object.entries((ESLintPluginZod as unknown as Plugin).rules)
...
const eslint_rule_descriptions = ([...builtinRules.entries()] as [string, Plugin['rules'][string]][]).filter(([, value]) => value.meta.docs.description).map(([name, rule]) => ({ name, description: rule.meta.docs.description }));
...
const zod_rules = Object.entries(ESLintPluginZod.configs.recommended.rules as Record<string, string | string[]>)
  ...
  .map(([name]) => [`[@${name}](https://github.com/marcalexiei/eslint-zod/blob/main/plugins/eslint-plugin-zod/docs/rules/${name.replaceAll('zod/', '')}.md)`, `${eslint_plugin_zod_rule_descriptions.find((rule) => rule.name === name.replaceAll('zod/', ''))?.description ?? 'None'}.`])
